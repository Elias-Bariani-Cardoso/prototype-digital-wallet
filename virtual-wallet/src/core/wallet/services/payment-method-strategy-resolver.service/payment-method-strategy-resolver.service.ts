...
@Injectable()
export class PaymentMethodStrategyResolverService {
  ...
  resolve(type: PaymentMethodType): PaymentMethodStrategy {
    const strategy = this.strategies.get(type);
    if (!strategy) {
      throw new Error(`Estratégia não implementada para o tipo ${type}`);
    }
    return strategy;
  }
}
